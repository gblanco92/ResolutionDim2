...
intrinsic WeierstrassEquation(s::RngSerPuisElt, Q::RngMPol) -> RngMPolElt
...
intrinsic IntegralClosure(I::RngMPol: StdBasis := false) -> RngMPol
...
require Rank(Parent(Representative(I))) eq 2:
  ...
  Q<x, y> := PolynomialRing(Parent(c[1]), 2);
  II := [ IntegralClosureImpl(K[1], K[2], K[3], Q) :
    K in ClusterFactorization(P, v, c) ];
  return ideal<Q | f>*CleanIdeal([i gt 1 select Self(i - 1)*II[i]
    else II[1] : i in [1..#II]][#II]); // Not working (ex: v4)
  // Move result to a local ring & compute std basis if requested.
  //R<x, y> := LocalPolynomialRing(CoefficientRing(Q), 2, "lglex");
  //J := ideal<R | [R | R!f : f in Generators(J)]>;
  //if StdBasis then J := ideal<R | StandardBasis(J)>; end if;
  //return J;
end intrinsic;
